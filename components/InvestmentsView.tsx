  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in duration-300">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <button onClick={() => { setTempCdiRate(cdiRate.toString()); setIsCdiModalOpen(true); }} className="text-accent text-sm font-medium flex items-center gap-1 hover:text-white transition-colors cursor-pointer group">
             {isFetchingCdi ? <span className="flex items-center gap-2 text-gray-400"><RefreshCw className="w-3 h-3 animate-spin" /> Atualizando...</span> : <>{cdiRate}% do CDI a.a. <Settings2 className="w-3 h-3 group-hover:rotate-90 transition-transform" /></>}
          </button>
        </div>
      </div>
      <div className="bg-accent rounded-[2.5rem] p-6 flex flex-col gap-4 relative overflow-hidden flex-shrink-0 shadow-xl shadow-black/40 mb-8">
          ...
          <div className="relative z-10"><span className="text-white/80 text-xs font-bold uppercase tracking-wider block mb-1">Patrimônio Total</span><h3 className="text-4xl font-bold text-white tracking-tight">R$ {totalInvested.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</h3></div>
          <div className="h-px bg-white/20 w-full relative z-10" />
          <div className="grid grid-cols-2 gap-3 relative z-10">
            <div className="bg-black/20 p-4 rounded-2xl backdrop-blur-sm"><span className="text-white/70 text-[10px] font-bold uppercase block mb-1">Rendimento (Mês)</span><p className="text-white font-bold text-lg">+ R$ {(estimatedYearlyReturn / 12).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p></div>
            <div className="bg-black/20 p-4 rounded-2xl backdrop-blur-sm"><span className="text-white/70 text-[10px] font-bold uppercase block mb-1">Total Estimado (Ano)</span><p className="text-white font-bold text-lg">+ R$ {estimatedYearlyReturn.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p></div>
          </div>
      </div>
      <div className="flex-1 flex flex-col gap-2">
          <div className="flex justify-between items-center px-1 mb-2"><h3 className="text-sm font-bold text-gray-400 uppercase">Seus Ativos</h3></div>
          {(investments || []).length === 0 ? <div className="flex flex-col items-center justify-center py-10 opacity-50"><Building className="w-12 h-12 text-gray-500 mb-2" /><p className="text-xs text-gray-400">Nenhum ativo cadastrado.</p></div> : (investments || []).map(inv => <SwipeableInvestmentItem key={inv.id} inv={inv} onEdit={(i) => { setEditingInvestment(i); setIsAddModalOpen(true); }} onDelete={onDelete} getYieldLabel={(i) => i.type === 'cdi' || i.type === 'fixed' ? `${i.yieldRate}% CDI` : `DY ${i.yieldRate}%`} />)}
      </div>
      ...
      <AddInvestmentModal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} onSave={(data) => editingInvestment ? onEdit({ ...data, id: editingInvestment.id }) : onAdd(data)} investmentToEdit={editingInvestment} isPro={isPro} onOpenProModal={onOpenProModal} />
      {isCdiModalOpen && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
           <div className="bg-[#1c1c1e] w-full max-w-xs rounded-[2rem] p-6 shadow-2xl border border-white/5 relative flex flex-col gap-4">
              ...
              <form onSubmit={(e) => { e.preventDefault(); const rate = parseFloat(tempCdiRate.replace(',', '.')); if (!isNaN(rate)) { onUpdateCdiRate(rate); setIsCdiModalOpen(false); } }} className="flex flex-col gap-4 mt-2">
                 ...
                 <button type="submit" className="flex-1 bg-accent text-black h-12 rounded-xl font-bold text-sm">Atualizar Taxa</button>
              </form>
           </div>
        </div>
      )}
