...
const CalendarModalBase: React.FC<Props> = ({ isOpen, onClose, transactions = [], activeMonthContext, appLanguage }) => {
            ...
            {selectedDayTransactions.length === 0 ? (
              ...
              selectedDayTransactions.map((tx) => (
                <div key={tx.id} className="flex items-center gap-3 p-3 rounded-2xl bg-[#1c1c1e] border border-white/5 shadow-sm shrink-0">
                  <div className={`w-1 h-8 rounded-full ${tx.paid ? 'bg-green-500' : (tx.type === 'subscription' ? 'bg-purple-500' : 'bg-cyan-400')
                    }`} />
...
export const CalendarModal = React.memo(CalendarModalBase);
